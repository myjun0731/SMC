...
<body>
	<form action="GetSnackUpdate_confirm.jsp" name="form">
		...
		<table border="1">
			...
			String snackList_sql = "select s.snack_name from snack s " + "order by s.snack_name";
			...
			<tr>
				...
				<td>
					<%
					while (jdbc2.rs.next()) {
						if (snack_name.equals(jdbc2.rs.getString(1))) {
					%> <input type="radio" name="snack_name"
					value="<%=jdbc2.rs.getString(1)%>" id="<%=jdbc2.rs.getString(1)%>"
					checked> <label for="<%=jdbc2.rs.getString(1)%>"><%=jdbc2.rs.getString(1)%></label>
					<%
					} else {
					%> <input type="radio" name="snack_name"
					value="<%=jdbc2.rs.getString(1)%>" id="<%=jdbc2.rs.getString(1)%>">
					<label for="<%=jdbc2.rs.getString(1)%>"><%=jdbc2.rs.getString(1)%></label>
					<%
					}
					}
					%>
